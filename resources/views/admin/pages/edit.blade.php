    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <form action="{{ route('admin.online-store.mnpages.update', $mnpage) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer">
                            ...
                            <input type="checkbox" name="is_home" value="1" {{ old('is_home', $mnpage->is_home) ? 'checked' : '' }} class="mt-0.5 h-4 w-4 border-gray-300 rounded text-black focus:ring-black">
                            <div>
                                <span class="text-sm font-bold text-gray-900 block">Primary Home Page</span>
                                @if($hasOtherHomePage && !$mnpage->is_home)
                    ...
                    <div class="pt-4">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <input type="checkbox" name="is_about_page" value="1" {{ old('is_about_page', $mnpage->is_about_page) ? 'checked' : '' }} class="mt-0.5 h-4 w-4 border-gray-300 rounded text-black focus:ring-black">
                            <div class="flex-1">
            ...
            <div class="pt-6 border-t border-gray-100 flex justify-between items-center">
                <div>
                    @if(!$mnpage->is_home)
                        <button type="button" onclick="if(confirm('Are you sure you want to delete this page? This action cannot be undone.')) document.getElementById('delete-page-form').submit();" class="text-red-600 hover:text-red-800 text-sm font-bold flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-red-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            Delete Page
                        </button>
                    @else
                        <span class="text-xs text-gray-400 italic px-3 py-2">Home Page cannot be deleted</span>
                    @endif
                </div>
        ...
        @if(!$mnpage->is_home)
            <form id="delete-page-form" action="{{ route('admin.online-store.mnpages.destroy', $mnpage) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        @endif
