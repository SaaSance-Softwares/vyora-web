...
export default function ProductDetailClient({ product, policies = {}, coupons = [] }: { product: ProductDetail; policies?: Record<string, string>; coupons?: any[] }) {
    ...
    const sanitizeUrl = (url: string | null | undefined): string | null => {
        ...
        // If it's still an absolute URL (e.g. backendUrl mismatch), return it directly to avoid prefixing with "/"
    ...
    return (
        <>
                        ...
                        <div>
                            ...
                            {(product as any).reviews_summary && (product as any).reviews_summary.total_reviews > 0 && (
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        {(product as any).reviews_summary.total_reviews} {((product as any).reviews_summary.total_reviews === 1) ? 'Review' : 'Reviews'}
                                    </span>
                                </div>
                            )}
            ...
            {/* ─── REVIEWS SECTION ────────────────────────────────── */}
            <div id="reviews-section" className="max-w-4xl mx-auto px-4 py-16 border-t border-gray-100">
                ...
                <div className="space-y-8">
                    {((product as any).reviews || []).map((review: any) => (
                        <div key={review.id} className="border-b border-gray-100 pb-8">
                            ...
                            {review.comment && (
                                <p className="text-gray-700 text-sm leading-relaxed mt-4">{review.comment}</p>
                            )}
                            ...
                            {review.admin_reply && (
                                <div className="mt-4 bg-gray-50 p-4 rounded-xl border-l-4 border-black">
                                    <p className="text-xs font-bold uppercase tracking-widest text-gray-900 mb-1">Store Reply</p>
...
function ReviewFormModal({ isOpen, onClose, product }: { isOpen: boolean, onClose: () => void, product: any }) {
    ...
    const submit = (e: React.FormEvent) => {
        ...
        post(`/products/${product.id}/reviews`, {
            preserveScroll: true,
            onSuccess: () => {
                // Flash success will trigger close
            }
        });
    };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="bg-white rounded-2xl w-full max-w-lg relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    <h2 className="text-xl font-bold text-gray-900 font-heading">Write a Review</h2>
                ...
                <div className="p-6 overflow-y-auto">
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        <div>
                            <label className="block text-sm font-bold text-gray-900 uppercase tracking-widest mb-3">Rating</label>
                            <div className="flex gap-2">
                                {[1, 2, 3, 4, 5].map(num => (
                                    ...
                                    >
                                        <Star className={`w-8 h-8 ${data.rating >= num ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
                                    </button>
                                ))}
                            </div>
                        ...
                        <div>
                            <label className="block text-sm font-bold text-gray-900 uppercase tracking-widest mb-2">Review (Optional)</label>
                            <textarea
                                value={data.comment}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-gray-900 uppercase tracking-widest mb-2">Add Photos (Optional)</label>
                            ...
                            <div className="flex flex-wrap gap-3">
                                ...
                                {previewUrls.length < 5 && (
                                    <label className="w-20 h-20 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 hover:border-black transition-colors text-gray-500 hover:text-black">
