...
export default function HeroSlider({ data, sectionSettings }: { data: any, sectionSettings?: any }) {
    const id = useId();
    ...
    const fitClass: any = { cover: 'object-cover', contain: 'object-contain', fill: 'object-fill' };
    const selectedFit = fitClass[data.image_fit || 'cover'] || 'object-cover';

    
    return (
        <div className={`w-full relative group/hero ${selectedHeight}`}>
            <Swiper
            ...
            >
                {data.slides.map((slide: any, index: number) => {
                    const SlideContent = (
                        <div className={`relative w-full ${isAutoHeight ? '' : 'h-full'}`}>
                            <img
                    ...
                    return (
                        <SwiperSlide key={index} className={`relative w-full ${isAutoHeight ? '' : 'h-full'}`}>
                            {slide.link ? (
                                <Link href={slide.link} className={`block w-full ${isAutoHeight ? '' : 'h-full'}`}>
                                    {SlideContent}
                                </Link>
                            ) : (
                                SlideContent
