...
function AddressCard({ addr, onDelete, onSetDefault, onEdit }: { addr: Address; onDelete: (id: number) => void; onSetDefault: (id: number) => void; onEdit: (addr: Address) => void }) {
    return (
        <div className={`relative p-4 rounded-xl border transition-all ${addr.is_default ? 'border-gray-900 bg-gray-50' : 'border-gray-200 bg-white'}`}>
            ...
            <div className="flex items-center gap-3 mt-3">
                ...
                <div className="flex items-center gap-3 ml-auto">
                    <button onClick={() => onEdit(addr)} className="text-[11px] font-semibold text-blue-500 hover:text-blue-700 transition-colors flex items-center gap-0.5">
                        <Pencil className="w-3 h-3" /> Edit
                    </button>
                    <button onClick={() => onDelete(addr.id)} className="text-[11px] font-semibold text-red-400 hover:text-red-600 transition-colors flex items-center gap-0.5">
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <main className="flex-1 space-y-6 min-w-0">
                    {activeTab === 'profile'     && <ProfileSection user={userData || user} onSaved={() => { useAuthStore.getState().checkAuth().then(() => api.get('/api/user').then(r => setUserData(r.data))); }} />}
